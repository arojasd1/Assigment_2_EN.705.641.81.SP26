...
class CustomModelforSequenceClassification(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        if self.type == "full":
            ...
            inputs_embeds = torch.cat([prefix, input_embeddings], dim=1).to(device='cuda')
            # Hint: you can use self.model.embeddings.word_embeddings to get the input embeddings
            prefix_mask = torch.ones(batch_size, self.prefix.size(0), device=attention_mask.device)
...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, test_dataloader, device, lr, model_name):
    ...
    if mymodel.type == "head":
        ...
        for param in mymodel.model.parameters():
    ...
    for epoch in range(num_epochs):
        ...
        for index, batch in tqdm(enumerate(train_dataloader)):
            ...
            # forward pass
            # name the output as `output`
            output = mymodel(input_ids=inputs_ids, attention_mask=attention_mask)
...
if __name__ == "__main__":
    ...
    val_acc = val_accuracy['accuracy']
    test_acc = test_accuracy['accuracy']

    print("\n===== FINAL RESULTS =====")
    print(f"Model: {args.model} ({args.type} fine-tuning)")
