...
### Runnning
`python base_classification.py --mode compare --device cuda --batch_size 64` for model comparison
`python base_classification.py --mode single --device cuda --model distilbert-base-uncased --batch_size 64 --lr 1e-4 --num_epochs 30` for single Run
`python base_classification.py --mode sweep --device cuda --model distilbert-base-uncased --batch_size 64` for running hyperparameters requested by the HW6.
